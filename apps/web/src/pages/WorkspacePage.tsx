...
export function WorkspacePage({ view }: WorkspacePageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f5ef] text-[#102018]">
      ...
      <section
        className={`mx-auto px-6 py-10 lg:px-8 lg:py-14 ${
          view === 'board' ? 'max-w-[112rem]' : 'max-w-7xl'
        }`}
      >
        <div className="mb-10">
        ...
        <div
          className={
            view === 'board'
              ? 'space-y-6'
              : 'grid items-start gap-6 lg:grid-cols-[20rem_minmax(0,1fr)]'
          }
        >
          {view === 'board' ? (
            <div className="flex flex-wrap items-end justify-between gap-5 rounded-[1.5rem] border border-emerald-950/10 bg-white px-5 py-4 shadow-sm sm:px-6">
              <div>
                ...
                <p className="mt-1 text-sm text-emerald-950/50">
                  Choose the team and project whose work you want to manage.
                </p>
              </div>
              {teams.length === 0 ? (
                <Link
                ...
                <label className="text-xs font-black tracking-wide text-emerald-900 uppercase">
                  ...
                  <select
                    ...
                    className="mt-2 block min-w-64 rounded-xl border border-emerald-950/10 bg-white px-4 py-3 text-sm font-bold normal-case outline-none focus:border-emerald-700"
                    disabled={working}
                    onChange={(event) =>
                      void handleSelectTeam(event.target.value)
                    }
